...
if (isset($_POST['book_btn'])) {
	...
	$personsBooking = mysqli_real_escape_string($db, $_POST['personsBooking']);
	$dateBooking = mysqli_real_escape_string($db, $_POST['dateBooking']);
	$hourBooking = mysqli_real_escape_string($db, $_POST['hourBooking']);

	if(empty($personsBooking)){
		array_push($errors, "Shenoni numrin e personave!");
	} 
	if(empty($dateBooking)) {
		array_push($errors, "Zgjedhni daten!");
	}
	if(empty($hourBooking)) {
		array_push($errors, "Shenoni oren!");
	}


	if(count($errors) == 0){
		$query = "insert into booking(persons, date, hour) values ('$personsBooking', '$dateBooking', '$hourBooking')";
